...
env="local"
files=''
path = "/Users/user/Desktop/homework/Bigdatawk/hw2-files.txt"
if env=="prod":
    path = './hw2-files.txt'
with open(path) as f:
...
# YOUR CODE HERE
twtRDD = rdd.map(lambda x: safe_parse(x)).filter(lambda x: x[0] != '0' or x[1] != 'INVALID').cache()
#raise NotImplementedError()
...
print_users_count
#raise NotImplementedError()
users_count = twtRDD.map(lambda x:x[0]).distinct().count()
...
with open("/Users/user/Desktop/homework/Bigdatawk/users-partition.pickle", 'rb') as f:
...
def getPartitionID(user):
    if user in partitions_dict:
        return partitions_dict[user]
    else:
        return 7
...
grp_count_rdd = twtRDD.map(lambda x: [getPartitionID(x[0]), 1]).reduceByKey(add).sortByKey()
...
# YOUR CODE HERE
#print_post_count(count_twt.collect())
counts_per_partition = print_post_count(grp_count_rdd.collect())
#raise NotImplementedError()
...
from operator import add
tokenizer = Tokenizer()
token_user_rdd = twtRDD.flatMap(lambda x: [[word, [x[0]]] for word in set(tokenizer.tokenize(x[1]))])                               .reduceByKey(add)                              .map(lambda x: [x[0], list(set(x[1]))]).cache()
...
frequent_token_rdd = token_user_rdd.map(lambda p: [p[0], len(p[1])])                                   .filter(lambda p: p[1] >= 100)                                   .sortBy(lambda p: p[1], ascending = False)
print_count(frequent_token_rdd)
print_tokens(frequent_token_rdd.take(20))
#raise NotImplementedError()


my_output.append("num-freq-tokens", frequent_token_rdd)
my_output.append("top-20-tokens", frequent_token_rdd.take(20))
print("Number of tokens:", frequent_token_rdd)
print_tokens(frequent_token_rdd.take(20))
...
token_relpop_rdd = token_user_rdd.filter(lambda p: len(p[1]) >= 100)                                 .map(lambda p : [p[0].encode("utf-8"), get_relpop_vec(p[1])])                                 .cache()

for k in range(8):
    group_token_relpop_rdd = token_relpop_rdd.map(lambda p: [p[0], p[1][k]])                                             .sortBy(lambda p: (-p[1], p[0]))
    print_tokens(group_token_relpop_rdd.take(10), k)


my_output.append("popular_10_in_each_group",twg.take(10))
...
#Following code add to your solution to 'my_output'

my_output.append("popular_10_in_each_group",twg.take(10)
for k in range(8):
    twg = group_token_relpop_rdd = token_relpop_rdd.map(lambda p: [p[0], p[1][k]])\
                                             .sortBy(lambda p: (-p[1], p[0]))
    print_tokens(twg.take(10), k)
timer_stop("Print popular tokens in each group")
